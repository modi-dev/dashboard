...
table=$(${oc_dir}/oc get pods -o jsonpath="{range .items[*]}{'<tr>'}{'<td align="left">'}{.metadata.labels.app}{'</td>'}{'<td align="left">'}{.spec.containers[0].image}{'</td>'}{'<td>'}{.metadata.creationTimestamp}{'</td>'}{'<td align="left">'}{range .spec.containers[*]}{.name}{'</br>'}{'<td>'}{'req.cpu: '}{.resources.requests.cpu}{'</td>'}{'lim.cpu: '}{.resources.limits.cpu}{'</br>'}{'<td>'}{'req.mem: '}{.resources.requests.memory}{'</td>'}{'lim.mem: '}{.resources.limits.memory}{'</br>'}{end}{'</td>'}{'<td>'}{.spec.containers[0].ports[0].containerPort}{'</td>'}{'<td>'}{.status.phase}{'</td>'}{'</tr>'}{end}")
## добавляем время и HTML разметку и заголовок таблицы
html="${html}<html><body><p>update time: ${date}<br>namespace: ${namespace}</p><br><table class='iksweb'><thead><tr><th>NAME</th><th>VERSION</th><th>CREATION DATE</th><th>REQUESTS</th><th>LIMITS</th><th>PORTS</th><th>STATUS</th></tr></thead>${table}</table></html>"
